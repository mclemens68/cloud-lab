...
resource "aws_route53_record" "ec2_public_dns" {
  ...
  name     = "${local.aws_config.dnsSubDomain}${each.key}"
  type     = "A"
  ttl      = "30"
...
resource "aws_route53_record" "ec2_private_dns" {
  ...
  zone_id  = data.aws_route53_zone.zone.zone_id
  name     = each.value["publicIP"] ? "${local.aws_config.dnsSubDomain}${each.key}-priv" : "${local.aws_config.dnsSubDomain}${each.key}"
  type     = "A"
  ttl      = "30"
